...
if ENV["SEED"]
  ...
  def create_user(email, username = Faker::Name.name, roles = [])
    ...
    User.create!(username: username, email: email, password: pwd,
                 ...
                 terms_of_service: "1", roles: roles)
  end
  ...
  puts "Creating participatory process"

  (0..1).each do |i|
    pp = ParticipatoryProcess.create!(name: Faker::Lorem.sentence(3).truncate(20),
                                      admin_name: Faker::Name.name,
                                      admin_email: Faker::Internet.email,
                                      areas: Faker::Lorem.sentence(3).truncate(60),
                                      title: {
                                        es: Faker::Lorem.sentence(3).truncate(20),
                                        en: Faker::Lorem.sentence(3).truncate(20),
                                        ca: Faker::Lorem.sentence(3).truncate(20)
                                      },
                                      subtitle: {
    ...
    puts "Creating Debates on Participatory Process #{pp.decorate.title}"
    ...
    (1..10).each do |i|
      ...
      debate = Debate.create!(author: author,
                              participatory_process_id: pp.id,
    ...
    puts "Creating Categories and Subcategories on Participatory Process #{pp.decorate.title}"
    (1..5).each do |n|
      name = "Axis #{n}"
      description = "<p>#{Faker::Lorem.paragraphs.join('</p><p>')}</p>"
      category = Category.create(name: { ca: name, es: name, en: name }, description: { ca: description, es: description, en: description }, position: n, participatory_process_id: pp.id)
      (1..5).each do |m|
    ...
    puts "Creating Proposals on Participatory Process #{pp.decorate.title}"
    ...
    puts "Creating Meetings on #{pp.decorate.title} Participatory Process"
