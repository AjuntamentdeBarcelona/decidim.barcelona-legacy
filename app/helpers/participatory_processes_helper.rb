module ParticipatoryProcessesHelper
  def participatory_process_select(form)
    ...
    form.select :participatory_process_id, collection, {} , data: { 'load-scoped-fields-url': load_scoped_fields_moderation_meetings_path(format: :js), 'meeting-id': @meeting.try(:id)}
  end
end
